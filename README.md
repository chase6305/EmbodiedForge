...
点任务、Go1 在线控制和 Go1/H1 回放共用灰蓝色场景风格：哑光棋盘地板、方向光和补光，MuJoCo 使用渐变天空，RTX 使用柔和棚拍光照。地板纹理固定在世界坐标中，便于判断移动距离。Raster 使用简化网格背景。这些样式仅用于查看器，不改变训练观测或物理参数；各渲染器的色调和阴影仍有差异。
...
| 相机与全屏 | 拖动旋转、滚轮缩放、默认视角、跟随机器人、全屏及截图；按后端能力启用 |
...
暂停且画面未变时复用 JPEG，不重复渲染和传输；控制状态仍更新。多个浏览器共享会话。断线后禁用操作并自动重连，不重发旧操作，也不会自动暂停服务端仿真。停止按钮或 Ctrl+C 释放会话资源。
